/**
 * 업로드 전 브라우저 리사이즈 — canvas 로 축소하고 JPEG 로 통일한다.
 *
 * 세 가지를 한 번에 해결한다:
...
/** JPEG 품질 — 육안 차이가 거의 없으면서 용량이 크게 준다 */
const QUALITY = 0.85;
...
export interface ResizedImage {
   ...
   * photo.heic 를 그대로 보내면 heic + image/jpeg 조합이 되어 거부된다.
   */
  filename: string;
}
...
/**
 * 파일명의 확장자를 .jpg 로 교체 (확장자가 없으면 붙인다).
...
function drawToBlob(
  ...
  // 투명 PNG 를 JPEG 로 바꾸면 투명 영역이 검게 나온다 — 흰 배경을 먼저 깐다
 ...
 * 업로드용으로 리사이즈한다.
 *
 * 목표치를 하나씩 낮춰가며 시도한다 — iOS Safari 의 canvas 메모리 한계는 기기마다
 * 달라서, 1000px 이 실패해도 500px 은 통과하는 경우가 있다. 화질이 조금 떨어져도
 * 아예 못 올리는 것보다 낫다(상품 카드는 300px 남짓으로 표시된다).
 *
...
export async function resizeImageForUpload(file: File): Promise<ResizedImage> {
  ...
  try {
    ...
    for (const maxSide of MAX_SIDES) {
      ...
      const blob = await drawToBlob(source, w, h);

...
허용 접두: `/seller` `/login` `/signup` **`/landing`**
...
장바구니 **변경 3종에만** 붙인다(조회 제외). BE `CartService` 가 이벤트를 적재할 때
방문 세션을 알 방법이 이 헤더뿐이다 — `member_id`·`guest_id` 는 JWT·쿠키에서 뽑지만
`sessionKey` 는 FE 가 localStorage 로 관리하는 값이라서.

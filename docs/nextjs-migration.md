...
> 작성 2026-07-28 · **상태: 1~5단계 코드 완료, 브라우저 검증 대부분 통과(전환 스위치 OFF).**
> **남은 것: 결제·챗봇 실사용 확인 → 워크플로 rename → 전환.** 원본 Vite 앱이 계속 배포 중이다.
> 검증 진행 상황은 `docs/nextjs-migration-qa.md` 상단 표 참조 (버그 3건 발견·수정됨)
